...
const ComDashboard = () => {
  ...
  useEffect(() => {
    const fetchScores = async () => {
      try {
        ...
        const totalScoreSum = groupedScores.reduce(
          (acc, score) => acc + score.score,
          0
        );
        ...
        const scoresWithPercentage = groupedScores.map((scoreData) => ({
          ...scoreData,
          percentage:
            totalScoreSum === 0 ? 0 : (scoreData.score / totalScoreSum) * 100,
        }));

        const sortedScores = scoresWithPercentage.sort(
          (a, b) => b.score - a.score
        );
  ...
  return (
    <div className="flex flex-col justify-center items-center max-w-screen min-h-screen bg-PC">
      <h3 className="text-center text-4xl text-amber-900 font-great mt-6">
        <b>Scores</b>
      </h3>
